...
angular.module('pbnApp')
	.controller('MainCtrl', function ($scope) {
		...
		$scope.imageLoaded = function (imgSrc) {
			...
			img.src = imgSrc;
			img.onload = function () {
				console.log("MainCtrl: 图片对象加载完成 (onload triggered)"); // 用于调试
				var c = document.getElementById("img-canvas");

				// 【建议修改】确保获取的是数字类型
				var widthVal = document.getElementById("widthSlider").value;
				c.width = parseInt(widthVal, 10) || 800;
				...
				// 切换步骤
				$scope.step = "select";
				console.log("MainCtrl: 步骤已更新为 select"); // 用于调试

				// 使用 $timeout 或者是安全的 apply
				if (!$scope.$$phase) {
					$scope.$apply();
				}
			};
